...
class FormCBlisterController extends GetxController {
  final Task task;
  final String brmNo;
  ...
  FormCBlisterController(this.task) : brmNo = task.brmNo ?? '';
  ...
  Future<void> fetchChildMaterials() async {
    try {
      ...
      final response = await Api.get('tasks/${task.id}/child-materials-blister');
  ...
  Future<void> fetchExistingData() async {
    try {
      isLoading.value = true;
      final response = await Api.get('form-c-blister/${task.id}');
  ...
  Future<bool> submitForm(Map<String, dynamic> formData) async {
    try {
      ...
      // Submit the data
      final response = await Api.post('form-c-blister', dataToSubmit);
