...
class FormCAssySyringeController extends GetxController {
  final Task task;
  final String brmNo;
  ...
  Future<void> fetchExistingData() async {
    try {
      isLoading.value = true;
      final response = await Api.get('form-c-assy-syringe/${task.id}');
  ...
  Future<bool> submitForm(Map<String, dynamic> formData) async {
    try {
      ...
      // Submit the data
      final response = await Api.post('form-c-assy-syringe', dataToSubmit);
