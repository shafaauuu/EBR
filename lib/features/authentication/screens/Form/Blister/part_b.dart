...
class _PartB_BlisterState extends State<PartB_Blister> {
  ...
  void check() async {
    ...
    await controller.fetchFormBBlisterData(widget.task.id.toString());
    setState(() {
      counter++;
    }); // triggers rebuild
              ...
              Center(
                child: ElevatedButton(
                    onPressed: () async {
                      ...
                      // Check if no machine was selected
                      if (payload.isEmpty) {
                        ArtSweetAlert.show(
                          context: context,
                      ...
                      try {
                        final response = await controller.submitQualificationDataBlister(payload);
                        if (response) {
                          ArtSweetAlert.show(
                            context: context,
                            artDialogArgs: ArtDialogArgs(
