...
class PartB_NeedleAssy extends StatefulWidget {
  final Task task;
  const PartB_NeedleAssy({super.key, required this.task});

  @override
  _PartB_NeedleAssyState createState() => _PartB_NeedleAssyState();
}

class _PartB_NeedleAssyState extends State<PartB_NeedleAssy> {
  ...
  void check() async {
    ...
    await controller.fetchFormBNeedleAssyData(widget.task.id.toString());
    setState(() {
      counter++;
    }); // triggers rebuild
              ...
              Center(
                child: ElevatedButton(
                  onPressed: () async {
                    ...
                    try {
                      final response = await controller.submitQualificationDataNeedleAssy(payload);
                      if (response) {
                        ArtSweetAlert.show(
                          context: context,
                          artDialogArgs: ArtDialogArgs(
