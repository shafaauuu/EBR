...
import 'package:oji_1/features/authentication/controller/Form/E/form_e_injection_controller.dart';
...
class PartE_Injection extends StatefulWidget {
  ...
  const PartE_Injection({super.key, required this.task});
...
class _PartE_InjectionState extends State<PartE_Injection> {
  ...
  final formEController = Get.put(FormEInjectionController());
  int counter = 0;
  ...
  void check() async {
    final controller = Get.put(FormEInjectionController());
    await controller.fetchFormEInjection(widget.task.id.toString());
    setState(() {
      counter++;
    }); // triggers rebuild
